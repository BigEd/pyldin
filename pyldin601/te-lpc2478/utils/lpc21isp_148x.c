...
static LPC_DEVICE_TYPE LPCtypes[] =
{
  ...
  { 0x1600f925, 2368, 512, 58, 27, 4096, SectorTable_213x }, // Anoter ID of LPC2368
  ...
  { 0x1600FF35, 2468, 512, 98, 27, 4096, SectorTable_213x }, //added by Embedded Artists AB
  { 0x1701FF35, 2478, 512, 98, 27, 4096, SectorTable_213x }  //added by Embedded Artists AB
};
...
static void ReadArguments(ISP_ENVIRONMENT *IspEnvironment, int argc, char *argv[])
{
    ...
    if(argc >= 5)
    {
        for(i = 1; i < (unsigned int)argc - 4; i++)
        {
...
int PhilipsDownload(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    *endPtr = '\0'; /* delete \r\n */
    ...
    if( LPCtypes[IspEnvironment->DetectedDevice].SectorTable[0] >= (int)IspEnvironment->BinaryLength )
    {
        Sector = 0;
        SectorStart = 0;
    }
