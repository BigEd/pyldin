...
static void rc_read(struct rc *rc, void(*error)(char *x))
{
...
static inline void rc_init_code(struct rc *rc, void(*error)(char *x))
{
	int i;
...
static void rc_do_normalize(struct rc *rc, void(*error)(char *x))
{
	if (rc->ptr >= rc->buffer_end)
		rc_read(rc, error);
	rc->range <<= 8;
...
static inline void rc_normalize(struct rc *rc, void(*error)(char *x))
{
...
static inline int rc_is_bit_0(struct rc *rc, uint16_t *p, void(*error)(char *x))
{
...
static int rc_get_bit(struct rc *rc, uint16_t *p, int *symbol, void(*error)(char *x))
{
...
/* Called once */
static inline int rc_direct_bit(struct rc *rc , void(*error)(char *x))
{
	rc_normalize(rc, error);
	rc->range >>= 1;
	if (rc->code >= rc->range) {
	...
	)
{
	...
	if (buf)
		...
		inbuf = large_malloc(LZMA_IOBUF_SIZE);
	if (!inbuf) {
